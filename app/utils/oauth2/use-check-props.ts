import type { TAuthTokenPayload, TOauth2Props } from "./types";

export const useCheckProps = <TData = TAuthTokenPayload>(
  props: TOauth2Props<TData>,
) => {
  const {
    authorizeUrl,
    clientId,
  ...
  if (typeof extraQueryParameters !== "object") {
    throw new TypeError(
      "extraQueryParameters must be an object for useOAuth2.",
    );
  }
