...
import { useMediaQuery } from "./use-stepper.ts";
      ...
      >
        ...
        >
          <Button
            ...
            disabled={!hasVisited}
            className={cn(
              "aspect-square h-8 w-8 rounded-full data-[highlighted=true]:bg-green-700 data-[highlighted=true]:border-green-700 data-[highlighted=true]:text-white",
...
const StepLabel = ({
  ...
  return shouldRender ? (
    <div
      aria-current={isCurrentStep ? "step" : undefined}
      className={cn(
        "flex w-max flex-col justify-center text-foreground font-light",
...
const Connector = React.memo(
  ({ isCompletedStep, children, isLastStep }: ConnectorProps) => {
    ...
    if (isVertical) {
      return (
        <div
          data-highlighted={isCompletedStep}
          className={cn(
            "ms-4 mt-1 flex h-auto min-h-[1rem] flex-1 self-stretch border-l-2 ps-8",
            isLastStep ? "min-h-0 border-transparent" : "",
            isCompletedStep ? "border-green-700" : "",
          )}
        >
          {!isCompletedStep && (
            <div className="block h-auto w-full">{children}</div>
          )}
        </div>
      );
    }
    ...
    return (
      <Separator
        data-highlighted={isCompletedStep}
        className="flex h-[2px] min-h-[auto] flex-1 self-auto data-[highlighted=true]:bg-green-700 data-[highlighted=true]:border-green-700"
